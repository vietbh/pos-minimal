...
final class CustomerController extends AbstractController
{
    ...
    { $this->denyAccessUnlessGranted(Permission::CUSTOMER_CREATE->value); $data=$this->data($request); if($request->isMethod('POST')){ $this->csrf($request,$csrf); try{$id=$handler(new CreateCustomerInput($data['name'],$data['phone']?:null,$data['note']?:null,$data['defaultDiscountPercent'])); $this->addFlash('success','Customer created.'); return $this->redirectToRoute('customers_show',['id'=>$id]);}catch(\Throwable $e){$this->addFlash('error',$this->safe($e));}} $response=$this->render('customer/form.html.twig',['customer'=>null,'data'=>$data]); if($request->isMethod('POST')){$response->setStatusCode(Response::HTTP_UNPROCESSABLE_ENTITY);} return $response; }
    ...
    { $this->denyAccessUnlessGranted(Permission::CUSTOMER_EDIT->value); $customer=$customers->findById($id); if(!$customer instanceof Customer) throw $this->createNotFoundException('Customer not found.'); $data=$this->data($request,$customer); if($request->isMethod('POST')){$this->csrf($request,$csrf); try{$handler(new UpdateCustomerInput($id,$data['name'],$data['phone']?:null,$data['note']?:null,$data['defaultDiscountPercent']));$this->addFlash('success','Customer updated.');return $this->redirectToRoute('customers_show',['id'=>$id]);}catch(\Throwable $e){$this->addFlash('error',$this->safe($e));}} $response=$this->render('customer/form.html.twig',['customer'=>$customer,'data'=>$data]); if($request->isMethod('POST')){$response->setStatusCode(Response::HTTP_UNPROCESSABLE_ENTITY);} return $response; }

    private function data(Request $r, ?Customer $c=null):array{return ['name'=>trim((string)$r->request->get('name',$c?->getName()??'')),'phone'=>trim((string)$r->request->get('phone',$c?->getPhone()??'')),'note'=>trim((string)$r->request->get('note',$c?->getNote()??'')),'defaultDiscountPercent'=>self::discountInput($r,$c?->getDefaultDiscountPercent()??0)];}
    private static function discountInput(Request $r,int $default): int { $raw=$r->request->get('defaultDiscountPercent',$default); return is_numeric($raw)?max(0,min(100,(int)$raw)):$default; }
