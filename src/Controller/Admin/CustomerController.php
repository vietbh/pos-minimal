...
final class CustomerController extends AbstractController
{
    ...
    { $this->denyAccessUnlessGranted(Permission::CUSTOMER_CREATE->value); $data=$this->data($request); if($request->isMethod('POST')){ $this->csrf($request,$csrf); try{$id=$handler(new CreateCustomerInput($data['name'],$data['phone']?:null,$data['note']?:null)); $this->addFlash('success','Customer created.'); return $this->redirectToRoute('customers_show',['id'=>$id]);}catch(\Throwable $e){$this->addFlash('error',$this->safe($e));}} return $this->render('customer/form.html.twig',['customer'=>null,'data'=>$data]); }

    #[Route('/app/customers/{id<\d+>}', name: 'customers_show', methods: ['GET'])]
    public function show(int $id, CustomerRepositoryInterface $customers): Response
    { $this->denyAccessUnlessGranted(Permission::CUSTOMER_VIEW->value); $customer=$customers->findById($id); if(!$customer instanceof Customer) throw $this->createNotFoundException('Customer not found.'); return $this->render('customer/show.html.twig',['customer'=>$customer]); }
    ...
    { $this->denyAccessUnlessGranted(Permission::CUSTOMER_EDIT->value); $customer=$customers->findById($id); if(!$customer instanceof Customer) throw $this->createNotFoundException('Customer not found.'); $data=$this->data($request,$customer); if($request->isMethod('POST')){$this->csrf($request,$csrf); try{$handler(new UpdateCustomerInput($id,$data['name'],$data['phone']?:null,$data['note']?:null));$this->addFlash('success','Customer updated.');return $this->redirectToRoute('customers_show',['id'=>$id]);}catch(\Throwable $e){$this->addFlash('error',$this->safe($e));}} return $this->render('customer/form.html.twig',['customer'=>$customer,'data'=>$data]); }
