...
final class DebtQueryRepository implements DebtQueryRepositoryInterface {
 ...
 public function listDebts(ListDebtsInput $input): DebtListResult {
  ...
  $rows=$qb->setFirstResult(($page-1)*$perPage)->setMaxResults($perPage)->getQuery()->getArrayResult(); $items=[]; foreach($rows as $r){$original=(string)$r['originalAmount'];$paid=$this->decimal((string)$r['paidAmount']);$remaining=$this->subtract($original,$paid);$items[]=new DebtListItemResult((int)$r['id'],(int)$r['customerId'],(string)$r['customerName'],(int)$r['orderId'],(string)$r['orderNumber'],$original,$paid,$remaining,$this->effectiveStatus($r['status'], $original, $paid),$this->date($r['createdAt']));} return new DebtListResult($items,$page,$perPage,$total);
 }
 public function findDebtById(int $id): ?DebtDetailResult {
  ...
  $d=$this->entityManager->createQueryBuilder()->select('d','c','o','dp')->from(Debt::class,'d')->join('d.customer','c')->join('d.order','o')->leftJoin('d.payments','dp')->where('d.id=:id')->setParameter('id',$id)->getQuery()->getOneOrNullResult(); if(!$d instanceof Debt||$d->getId()===null)return null;
  ...
  return new DebtDetailResult($d->getId(),$d->getCustomer()->getId()??0,$d->getCustomer()->getName(),$d->getCustomer()->getPhone(), $d->getOrder()->getId()??0,$d->getOrder()->getOrderNumber()->value(),$d->getOriginalAmount()->toDecimal(),$paid,$d->getRemainingAmount()->toDecimal(),$this->effectiveStatus($d->getStatus(), $d->getOriginalAmount()->toDecimal(), $paid),$d->getCreatedAt(),$payments);
 }
 private function effectiveStatus(mixed $storedStatus,string $original,string $paid):string{ $stored=is_object($storedStatus)?$storedStatus->value:(string)$storedStatus; if($stored==='REVERSED')return 'REVERSED'; $o=\App\Domain\Shared\ValueObject\Money::fromDecimal($original); $p=\App\Domain\Shared\ValueObject\Money::fromDecimal($paid); if($p->isZero())return 'OPEN'; if($p->isGreaterThanOrEqual($o))return 'PAID'; return 'PARTIALLY_PAID'; }
