...
final class PaymentBankAccountRepository implements PaymentBankAccountRepositoryInterface {
 ...
 public function findByWebhookToken(string $token):?PaymentBankAccount{return $this->em->createQueryBuilder()->select('a')->from(PaymentBankAccount::class,'a')->where('a.webhookToken = :token')->setParameter('token',$token)->setMaxResults(1)->getQuery()->getOneOrNullResult();}
